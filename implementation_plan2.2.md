...
#### H. Metadata Flags (NOT model features — used for row filtering and dashboard)
```
C4H6_Bottom_stuck          -> exclude from Model B training
C4H8_Bottom_stuck          -> exclude from Model A training
hours_since_*_change       -> drives Analyzer_Health; dashboard indicator
Analyzer_Health (GOOD/WARNING/BAD) -> dashboard badge; NOT a model feature
                               (would leak future info about analyzer state)
is_extreme_event           -> to be flagged in feature_engineering.py;
                               analyze separately; decide include/exclude
```
...
| `data/clean_data.parquet` | 11,343 rows x 24 columns — final | **Done** |
