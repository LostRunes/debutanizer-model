...
| **Tuned XGBoost** | **0.7037** | **0.9074** | **0.0516 wt%** | [model_A_XGBoost_robust_opt.json](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/models/model_A_XGBoost_robust_opt.json) |
...
### 4. Robust Hyperparameter Optimization (XGBoost)
* **Optimization Space**: We tuned `XGBRegressor` over 50 Optuna trials using 5-fold TimeSeriesSplit CV on Train Blocks 1-3.
* **Best parameters**:
  - `n_estimators`: 102
  - `max_depth`: 3
  ...
  - `reg_alpha`: 0.00078
  - `reg_lambda`: 3.76e-8
* **The Insight**: The optimal `max_depth` is extremely shallow ($3$). This physically constraints the trees to prevent overfitting and guarantees high generalization robustness.
* **Tuned Diagnostic Plots**:
  - [robust_opt_plot_1_actual_vs_predicted.png](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/experiments/diagnostics/robust_opt_plot_1_actual_vs_predicted.png)
  - [robust_opt_plot_4_residual_vs_time.png](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/experiments/diagnostics/robust_opt_plot_4_residual_vs_time.png)

---

## 7. File Inventory & Execution Walkthrough (Models A & B)

To run, reproduce, or deploy the soft-sensor systems, follow the file guide and execution sequence below:

### Release Folder: [final_v1/](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/final_v1/)
*   `models/` — Contains Model A JSON and PKL models, tuning results, and diagnostics plots.
*   `configs/` — Contains feature configuration JSONs for both Model A and Model B.
*   `reports/` — Contains Model A feature importance CSV and final summary report.
*   `inference/` — Contains online prediction scripts with fallback logic (`predict_c4h8.py`, `predict_c4h6.py`, and unified `predict_total_c4.py`).
*   `notebooks/` — Contains validation scripts (`verify_anchor_leakage.py` and `model_b_inversion_check.py`).

### File Inventory
1.  **[data_preprocessing.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/data_preprocessing.py)**: Cleans the raw Excel data, handles winsorization, flags stuck analyzer periods and shutdown rows.
2.  **[feature_engineering.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/feature_engineering.py)**: Handles gap-aware time-series resampling, calculates ratios, 24h deviations, and pressure-normalizations.
3.  **[notebooks/tune_robust_xgb.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/tune_robust_xgb.py)**: Runs the Optuna hyperparameter optimization CV for Model A on Train Blocks 1-3.
4.  **[notebooks/verify_anchor_leakage.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/verify_anchor_leakage.py)**: Formally proves Model A's anchor is leak-free and verifies R² = 0.7694 on Block 3 test split.
5.  **[notebooks/run_robust_checks.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/run_robust_checks.py)**: Generates robust split check statistics and diagnostic plots for Model A.
6.  **[notebooks/freeze_model_A.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/freeze_model_A.py)**: Pickles Model A and generates the final release artifacts.
7.  **[notebooks/model_b_target_audit.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/model_b_target_audit.py)**: Outputs C4H6_Bottom target statistics block by block to diagnose campaign operating mismatches.
8.  **[notebooks/model_b_anchor_audit.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/model_b_anchor_audit.py)**: Assesses C4H6 anchor coverage and age statistics in Block 4.
9.  **[notebooks/anchor_only_baselines.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/anchor_only_baselines.py)**: Evaluates deterministic C4H6 baseline predictions, revealing the 12h anchor alone scores R² = 0.9606.
10. **[notebooks/model_b_delta_model.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/model_b_delta_model.py)**: Fits and evaluates a delta correction model for C4H6, proving process variables degrade the raw anchor prediction.
11. **[notebooks/model_b_inversion_check.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/notebooks/model_b_inversion_check.py)**: Runs robustness checks for the C4H6 12h anchor across Blocks 2, 3, and 4.
12. **[inference/predict_total_c4.py](file:///c:/Users/KIIT/OneDrive/Desktop/DEBUTANIZER-model/inference/predict_total_c4.py)**: The unified online prediction entry point that combines Model A ML and Model B anchor prediction to compute Total C4 composition.

### Order of Execution
Execute the pipeline in the following sequence to fully clean, build, audit, and verify both models:
...
# Step 3: Train and tune Model A (XGBoost)
python notebooks/tune_robust_xgb.py

# Step 4: Verify Model A leakage & Block 3 metrics
...
# Step 6: Audit Model B target distributions block by block
...
# Step 8: Run Model B anchor-only baseline performance checks
python notebooks/anchor_only_baselines.py

# Step 9: Verify process delta models degrade raw anchor predictions
...
# Step 11: Test combined DCS online inference calculations
